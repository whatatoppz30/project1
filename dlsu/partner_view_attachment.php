...
$slect="SELECT * FROM `tblpartnerlog` WHERE partnerid='$_GET[partnerid]' AND partnerlogid='$_GET[logid]'";
 $runslect=mysqli_query($conn, $slect);

 while ($row = mysqli_fetch_assoc($runslect))
 {
  if ($row['action']=='CREATED' AND $row['relevantdoc'] !='')
{
...
else
{
    ...
    $slect="SELECT * FROM `tblpartnerattachment` WHERE partnerid ='$_GET[partnerid]' AND logid='$_GET[logid]' ";
 $runslect=mysqli_query($conn, $slect);
 ...
 {
echo ' <div class="col-md-6 col-xl-3" data-category="claims">
                                <a class="block block-rounded block-link-shadow"  href="assets/media/photos/'.$row['attachment'].'" download>
                                    <div class="block-content block-content-full text-center">
                                        <div class="item item-circle bg-info-light text-info mx-auto my-20">
                                            <i class="fa fa-file-text-o"></i>
                                        </div>
                                        <div class="font-size-lg">'.$row['attachment'].'</div>
                                        <div class="font-size-sm text-muted">-</div>
                                    </div>
                                </a>
                            </div>';
